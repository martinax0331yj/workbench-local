...
import { FileEdit, Edit3, Plus, X, Trash2, ChevronRight, Eye, Heart } from 'lucide-react';
...
const initForm = { title: '', summary: '', status: 'inspiration', publishDate: '', views: '', likes: '' };

export default function WechatPage() {
  const { wechatArticles, addWechatArticle, updateWechatArticle, deleteWechatArticle } = useStore();
  ...
  const selected = wechatArticles.find(a => a.id === selectedId) || null;
  ...
  const openEdit = (a: any) => {
    setEditingId(a.id);
    setForm({
      title: a.title, summary: a.summary || '', status: a.status || 'inspiration',
      publishDate: a.publishDate || '', views: a.views ? String(a.views) : '',
      likes: a.likes ? String(a.likes) : '',
    });
    setShowForm(true);
  };
  ...
  const handleSubmit = () => {
    ...
    const base = {
      ...
      status: form.status, publishDate: form.publishDate || undefined,
      views: form.views ? Number(form.views) : undefined,
      likes: form.likes ? Number(form.likes) : undefined,
    };
    if (editingId) {
      updateWechatArticle(editingId, base as any);
    } else {
      addWechatArticle({ ...base, outline: '', draft: '', tags: [] } as any);
    }
    ...
    <div>
      <div className="page-header flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <button onClick={openCreate} className="btn-primary flex items-center gap-1.5">
          <Plus size={16} /> <span>新建文章</span>
        </button>
      </div>

      <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 sm:gap-3 mb-4">
        {[
          { label: '文章总数', value: wechatArticles.length },
          { label: '已发布', value: published.length },
          { label: '修改中', value: wechatArticles.filter(a => a.status === 'revising').length },
          { label: '待发布', value: wechatArticles.filter(a => a.status === 'ready').length },
          { label: '灵感', value: wechatArticles.filter(a => a.status === 'inspiration').length },
          { label: '已复盘', value: wechatArticles.filter(a => a.status === 'reviewed').length },
        ].map(s => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-1 space-y-2 max-h-[70vh] overflow-y-auto scrollbar-thin pr-1">
          {wechatArticles.length === 0 ? (
            <p className="text-body-sm text-text-muted text-center py-8">暂无文章</p>
          ) : wechatArticles.map(a => (
            <div key={a.id} onClick={() => setSelectedId(a.id)}
              className={`card cursor-pointer transition-all hover:shadow-md ${selectedId === a.id ? 'ring-2 ring-warm-brown/30 border-warm-brown/30' : ''}`}>
              <div className="flex items-start justify-between gap-2">
                <h3 className="text-sm font-semibold text-text-primary truncate flex-1">{a.title}</h3>
                <ChevronRight size={14} className="text-text-muted flex-shrink-0 mt-0.5" />
              </div>
              {a.summary && <p className="text-caption text-text-muted line-clamp-2 mt-1">{a.summary}</p>}
              <div className="flex items-center gap-2 mt-2">
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${statusColors[a.status] || 'bg-gray-100'}`}>{statusLabels[a.status] || a.status}</span>
                {a.status === 'published' && a.publishDate && <span className="text-[10px] text-text-muted">{formatDateShort(a.publishDate)}</span>}
              </div>
            </div>
          ))}
        </div>
          ...
          {selected ? (
            <div className="card">
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  {selected.summary && <p className="text-sm text-text-secondary mt-1">{selected.summary}</p>}
                </div>
              ...
              <div className="space-y-3">
                ...
                {selected.status === 'published' && (
                  <div className="flex items-center gap-4 text-sm text-text-muted">
                    <span className="flex items-center gap-1"><Eye size={14} /> {(selected as any).views || 0} 阅读</span>
                    <span className="flex items-center gap-1"><Heart size={14} /> {(selected as any).likes || 0} 点赞</span>
            ...
            <div className="card flex items-center justify-center py-16">
              <div className="text-center">
                <FileEdit size={32} className="mx-auto text-text-muted mb-3" />
                <p className="text-body-sm text-text-muted">{wechatArticles.length > 0 ? '选择一篇文章查看详情' : '暂无文章，点击右上角新建'}</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-12 sm:pt-24 px-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl border border-gray-100 w-full max-w-md max-h-[80vh] overflow-y-auto">
            <div className="sticky top-0 bg-white rounded-t-2xl border-b px-5 py-3.5 flex items-center justify-between">
              <h3 className="font-semibold text-text-primary">{editingId ? '编辑文章' : '新建文章'}</h3>
            ...
            <div className="p-5 space-y-3.5">
              ...
              <div>
                <label className="block text-xs font-medium text-text-secondary mb-1">摘要</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field label="阅读量" value={form.views} onChange={v => setForm(p => ({ ...p, views: v }))} type="number" />
      ...
      {delId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl border border-gray-100 w-full max-w-sm p-6">
            ...
            <p className="text-sm text-text-secondary mb-4">确定要删除此文章吗？</p>
            <div className="flex gap-3">
              ...
              <button onClick={() => { deleteWechatArticle(delId); setDelId(null); setSelectedId(null); }}
